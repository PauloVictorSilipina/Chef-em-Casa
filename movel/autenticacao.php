...
function autenticar($db_con) {
	...
	if(!is_null($login)) {
		...
		$consulta = $db_con->prepare("SELECT senha FROM USUARIO WHERE email='$login'"); //Inicialmente estava token no lugar de senha e login no email
		$consulta->execute();
		...
		if($consulta->rowCount() > 0){
			...
			if(password_verify($senha, $linha['senha'])){ //Inicialmente estava token no lugar de senha 
				return true;
			}
		}
	}
